...
class GraspSelector(LeafSystem):
    ...
    def compute_grasp_cost(self, obj_pc_centroid, X_OG, t, name):
        """
        ...
        final_cost = 5*alignment + direction + 10*distance_obj_pc_desired_to_X_OG_y_axis

        return final_cost, distance_obj_pc_desired_to_X_OG_y_axis, direction, alignment


    def compute_candidate_grasps(self, obj_pc, obj_pc_centroid, obj_catch_t, candidate_num=int(np.random.uniform(100,800))):
        """
        Args:
            - obj_pc (PointCloud object): pointcloud of the object.
        ...
        def compute_candidate(random_center, direction_axis, obj_pc, kdtree, ball_radius, candidate_lst_lock, candidate_lst_1, candidate_lst_2):
            magnitude = np.random.uniform(0.2, 0.4)
            first_grasp_point = random_center - direction_axis * magnitude
            second_grasp_point = random_center + direction_axis * magnitude
            ...
            X_OF_1 = self.compute_darboux_frame(index_1, obj_pc, kdtree, ball_radius)  # find Darboux frame of random point
            X_OF_2 = self.compute_darboux_frame(index_2, obj_pc, kdtree, ball_radius)
            ...
            new_X_OG_cost_1, grasp_CoM_cost_1, direction_cost_1, collision_cost_1 = self.compute_grasp_cost(obj_pc_centroid, new_X_OG_1, obj_catch_t, 'gripper1')
            new_X_OG_cost_2, grasp_CoM_cost_2, direction_cost_2, collision_cost_2 = self.compute_grasp_cost(obj_pc_centroid, new_X_OG_2, obj_catch_t, 'gripper2')
    ...
    def SelectGrasp(self, context, output):
        if self.selected_grasp1_obj_frame is None and self.selected_grasp2_obj_frame is None:
            ...
            self.obj_reachable_start_t = 0.3  # random guess
            self.obj_reachable_end_t = 0.8  # random guess
            search_times = np.linspace(0, 1, 40)  # assuming first half of trajectory is definitely outside of iiwa's work envelope
            ...
            # obj_catch_t = 0.45*(self.obj_reachable_start_t + self.obj_reachable_end_t)  #0.365 0.4, 0.45
            obj_catch_t = np.random.uniform(self.obj_reachable_start_t, self.obj_reachable_end_t)
            ...
            #     print(self.obj_pose_at_catch)
                ...
                # body = self.plant.GetBodyByName('body', model_instance_index)

                # # To get the pose of this body in the world frame
