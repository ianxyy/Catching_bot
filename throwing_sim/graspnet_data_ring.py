    ...
    def data_collect(self, context, state):
        ...
        #     self.traj_count += 1
            ...
            if context.get_time() >= 0.8 and self.write:
                ...
                self.write_data_to_h5py(self.traj, self.pointcloud_input, self.pointcloud_normal , X_WG1_rotation, X_WG1_translation, X_WG2_rotation, X_WG2_translation,\
                 result, initial_pos)
                self.clear_data_buffers()

    
    def write_data_to_h5py(self, traj, pointcloud, normals, X_WG1_rotation, X_WG1_translation, X_WG2_rotation, X_WG2_translation,\
                 result, initial_pos):
        if result == 1:
        # Create or open an HDF5 file
            with h5py.File('graspnet9_data_5pc_ring_matrix.h5', 'a') as hf:
                # Check if datasets exist, and get the new index
                index = len(hf.keys()) // 10
                #input
            ...
            #     hf.create_dataset(f'obj_catch_t_{index}', data=np.array(self.obj_catch_t))        #1
            #     hf.create_dataset(f'result_{index}', data=np.array(result))                  #1
            with h5py.File('graspnet9_data_ring_clear.h5', 'a') as hf:
                # Check if datasets exist, and get the new index
                index = len(hf.keys()) // 10
                #input
                hf.create_dataset(f'traj_data_{index}', data=np.array(traj))     #5x150x15
        ...
        with open('graspnet9_data_result_5pc_ring_matrix.txt', "a") as text_file:
