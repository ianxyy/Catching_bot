        ...
        if self.traj_count < 5:
            ...
            if self.selected_grasp1_world_frame == None:
                with torch.no_grad():
                    ...
                    pointnet_model.load_state_dict(torch.load('model/XW_ring_matrix_2/pointnet_model_weights.pth', map_location=torch.device('cuda')))
                    transformer_model.load_state_dict(torch.load('model/XW_ring_matrix_2/transformer_model_weights.pth', map_location=torch.device('cuda')))
                    mlp_model.load_state_dict(torch.load('model/XW_ring_matrix_2/mlp_model_weights.pth', map_location=torch.device('cuda')))
                    ...
                    traj_pos_scaler_path = 'model/XW_ring_matrix_2/traj_pos_scaler.joblib'
                    traj_vel_scaler_path = 'model/XW_ring_matrix_2/traj_vel_scaler.joblib'
                    pc_scaler_path = 'model/XW_ring_matrix_2/pc_scaler.joblib'
                    X_WG1_scaler_path = 'model/XW_ring_matrix_2/X_WG1_scaler.joblib'
                    X_WG2_scaler_path = 'model/XW_ring_matrix_2/X_WG2_scaler.joblib'
                    ...
                    print(f'------------data-preprocessing time:{preprocess_time - start_time}')
                    xw_1_rot, xw_2_rot, xw_1_tran, xw_2_tran, obj_catch_t = mlp_model(combined_features)
                    inference_time = time.time()
                    print(f'------------infer time:{inference_time - preprocess_time}')
