...
class GraspDataset(Dataset):
    def __init__(self, h5_file, mode = 'train', limit=None):
    ...
    def compute_normalization_parameters(self):
        with h5py.File(self.h5_file, 'r') as hf:
            ...
            all_X_OG1_data = []
            all_X_OG2_data = []
            num_samples = min(len(hf.keys()) // 14, self.limit if self.limit else float('inf'))
            ...
            # Save scaler 
            dump(self.traj_pos_scaler, 'model/transformer_ablation/traj_pos_scaler.joblib')
            dump(self.traj_vel_scaler, 'model/transformer_ablation/traj_vel_scaler.joblib')
            ...
            dump(self.pc_scaler, 'model/transformer_ablation/pc_scaler.joblib')
            dump(self.X_WG1_pos_scaler, 'model/transformer_ablation/X_WG1_scaler.joblib')
            dump(self.X_WG2_pos_scaler, 'model/transformer_ablation/X_WG2_scaler.joblib')
            dump(self.X_OG1_pos_scaler, 'model/transformer_ablation/X_OG1_scaler.joblib')
            dump(self.X_OG2_pos_scaler, 'model/transformer_ablation/X_OG2_scaler.joblib')
...
def train(pointnet_model, transformer_model, mlp_model, optimizer, criterion, trainloader, batch_size, log=True):
    ...
    for batch_idx, batch in enumerate(trainloader):
        ...
        xw_1_rot, xw_2_rot, xw_1_tran, xw_2_tran, obj_catch_t = mlp_model(transformer_output_agg_flat)
...
def test(pointnet_model, transformer_model, mlp_model):
    ...
    pointnet_model.load_state_dict(torch.load('model/transformer_ablation/pointnet_model_weights.pth', map_location=torch.device('cuda')))
    transformer_model.load_state_dict(torch.load('model/transformer_ablation/transformer_model_weights.pth', map_location=torch.device('cuda')))
    mlp_model.load_state_dict(torch.load('model/transformer_ablation/mlp_model_weights.pth', map_location=torch.device('cuda')))

    pointnet_model.eval()
    transformer_model.eval()
    mlp_model.eval()  # Evaluation mode    

    traj_pos_scaler_path = 'model/transformer_ablation/traj_pos_scaler.joblib'
    traj_vel_scaler_path = 'model/transformer_ablation/traj_vel_scaler.joblib'
    pc_scaler_path = 'model/transformer_ablation/pc_scaler.joblib'
    X_WG1_scaler_path = 'model/transformer_ablation/X_WG1_scaler.joblib'
    X_WG2_scaler_path = 'model/transformer_ablation/X_WG2_scaler.joblib'
    X_OG1_scaler_path = 'model/transformer_ablation/X_OG1_scaler.joblib'
    X_OG2_scaler_path = 'model/transformer_ablation/X_OG2_scaler.joblib'
...
#     torch.save(pointnet_model.state_dict(), '/home/haonan/Catching_bot/throwing_sim/model/transformer_ablation/pointnet_model_weights.pth')
#     torch.save(transformer_model.state_dict(), '/home/haonan/Catching_bot/throwing_sim/model/transformer_ablation/transformer_model_weights.pth')
#     torch.save(mlp_model.state_dict(), '/home/haonan/Catching_bot/throwing_sim/model/transformer_ablation/mlp_model_weights.pth')
...
#     plt.legend()
#     # plt.show()
#     plt.savefig('transformer_ablation_noodle_training_validation_losses.png')
